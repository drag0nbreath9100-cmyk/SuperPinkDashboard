...
import { LayoutGrid, Users, Activity, Settings, Menu, LogOut, ChevronRight, User, ShieldCheck } from "lucide-react";
...
export function Sidebar() {
    ...
    // If on login page, don't show sidebar (though middleware should redirect, this is double safety/visual cleanliness)
    ...
    const coachNavItems = [
        ...
        // { icon: Dumbbell, label: "Workouts", href: "/coach/workouts" },
    ];
    ...
    return (
        <aside
            className={cn(
                "fixed left-0 top-0 h-full border-r border-white/5 bg-[#020617]/90 backdrop-blur-xl z-50 transition-all duration-300 ease-out flex flex-col py-8 overflow-hidden backface-hidden transform-gpu will-change-[width,box-shadow]",
                isHovered ? "w-72 shadow-2xl shadow-blue-900/20" : "w-24" // Expanded vs Collapsed width
            )}
        ...
        >
            ...
            <div className="flex items-center gap-4 px-6 mb-12">
                ...
                <div className={cn("transition-opacity duration-300 whitespace-nowrap overflow-hidden", isHovered ? "opacity-100" : "opacity-0 w-0")}>
                ...
                {navItems.map((item, idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={cn("font-medium whitespace-nowrap transition-all duration-300 origin-left", isHovered ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4")}>
                                {item.label}
                            </span>
            ...
            <div className="mt-auto px-4 flex flex-col gap-2">
                <form action={logout}>
                    ...
                    >
                        ...
                        <span className={cn("whitespace-nowrap transition-all duration-300", isHovered ? "opacity-100" : "opacity-0 w-0")}>Sign Out</span>
                    </button>
                </form>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 mx-2">
                    ...
                    >
                        <Link href="/profile" className="flex items-center gap-3 hover:bg-white/5 p-2 rounded-xl transition-all cursor-pointer relative z-10">
                            <div className="w-10 h-10 min-w-[2.5rem] rounded-full border border-primary/30 bg-slate-900 overflow-hidden shadow-[0_0_15px_hsl(var(--primary)/0.3)] flex items-center justify-center relative">
                                {/* We can potentially fetch existing image here too if we want, but keeping it simple with gradient for now or maybe use user image if available in future */}
                                <div className="w-full h-full bg-gradient-to-br from-primary to-purple-600" />
                            </div>
                            <div className={cn("transition-all duration-300 overflow-hidden", isHovered ? "opacity-100 w-auto" : "opacity-0 w-0")}>
                        ...
                        {/* Animated Analytics Popover via Portal */}
                                ...
                                {isHovered && statsOpen && (
                                    <motion.div
                                        ...
                                        style={{
                                            position: 'fixed',
                                            left: portalPos.left,
                                            bottom: portalPos.bottom - 20, // Slight offset adjustment to align nicely
                                            zIndex: 9999
                                        }}
                                        className="w-72 bg-[#0F172A]/90 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_0_40px_-10px_rgba(0,0,0,0.5)] p-0 overflow-hidden pointer-events-none"
                                    >
