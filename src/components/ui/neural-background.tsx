...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    return vec4(buf[0].x * uBrightness, buf[0].y * uBrightness, buf[0].z * uBrightness, 1.0);
  }
...
const CPPNShaderMaterial = shaderMaterial(
    { iTime: 0, iResolution: new THREE.Vector2(1, 1), uBrightness: 1.0 },
    vertexShader,
    fragmentShader
);
...
function ShaderPlane({ brightness = 0.5 }: { brightness?: number }) {
...
export function NeuralBackground({
    ...
    // We already have a good setup in the hero but let's make it static/customizable
    const camera = useMemo(() => ({ position: [0, 0, 1] as [number, number, number], fov: 75, near: 0.1, far: 1000 }), []);

    // If user wants dynamic entry animation, we can keep it.
    // For now, let's just set the initial state to the desired blur.
    useGSAP(
        () => {
        ...
        { scope: canvasRef, dependencies: [blurAmount] } // Re-run if blur changes?
    );

    return (
        <div className={`fixed inset-0 -z-50 w-full h-full bg-black ${className}`}>
            <div ref={canvasRef} className="w-full h-full">
                <Canvas
                    camera={camera}
                    gl={{ antialias: false, alpha: false, powerPreference: "high-performance" }} // Optimize
                    dpr={[1, 1.5]} // Cap DPR for performance
                    style={{ width: '100%', height: '100%' }}
                >
                    <ShaderPlane brightness={brightness} />
                </Canvas>
            </div>
            {/* Gradient Overlay for extra depth/darkness */}
